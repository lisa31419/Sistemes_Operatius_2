...
void *consumidor(void *arg) {
    ...
    int invalid, index_origin, index_destination, i;

    struct parametres *par = (struct parametres *)arg;

    //Fem locals les variables que venen per el parametre

    int **num_flights = par->num_flights;
    char **airports = par->airports;
    struct cell **buffer= par->buffer;
    int *comptador = par->comptador;
    int *r = par->r;

    struct cell *cell_consumer, *temp;

    //Inicialitzem memoria per copiar el contingut del buffer aquí per processar les dades


    cell_consumer= (struct cell *) calloc(1, sizeof(struct cell));
    cell_consumer->lines = (char **) calloc(NUMLINES, sizeof(char *));

    for (i = 0; i < NUMLINES; i++)
    {
  	  cell_consumer->lines[i] = (char *) calloc(MAXCHAR, sizeof(char));
    }
    
    
    // Les condicions que hem implementat ens funcionen millor que els nelems = 0
    // Mentre que el compatador no arribi a 0 ni tot el fitxer no s'ha llegit 
    // i possat al buffer, farem el seguent
    while (!finished && comptador != 0) {
    	
    	pthread_mutex_lock(&mutexP);

    	while ((*comptador)<= 0 && finished == false)  {
    		pthread_cond_wait(&condC, &mutexP);
    	}

    	if (((*comptador)<= 0) && finished == true) {
        pthread_mutex_unlock(&mutexP);
        break;
      }

      	// Canviem els punters per recollir la cel·la i processar-la
    	temp = cell_consumer;
    	cell_consumer = buffer[(*r)];
    	buffer[(*r)] = temp;
...
{  
  ...
  //S'ha de produir el buffer que emmagatzema els B blocs de N línies.
  struct cell **buffer;
  char line[MAXCHAR];
  buffer = calloc(BLOCKS, sizeof(struct cell*));
 

  for (i = 0; i < BLOCKS; i++) { 
  	buffer[i] = calloc(1, sizeof(struct cell));
  	buffer[i]->lines = calloc(NUMLINES, sizeof(char *));

  	for (j = 0; j < NUMLINES; j++) {
  		buffer[i]->lines[j] = calloc(MAXCHAR, sizeof(char));
  	}

  }


  //Inicializtem les variables en productor i pasem les referencies de memoria al struct per a que tant productor
  //com consumidor puguin modificar comptador que indicara els blocs pendent de lectura, r es només per a que els 
  //multiples fils de consumidors tinguin la mateixa referencia a quin bloc procesar.
  
  int comptador = 0;
  int r = 0;


  //Pasem totes les variables necessaries per als fils secundaris
  struct parametres par;
  par.buffer = buffer;
  par.num_flights = num_flights;
  par.airports = airports;
  par.comptador = &comptador;
  par.r = &r;
  ...
  //Inicialitzem espai per construir els blocs que asignarem al buffer.

  struct cell *cell_producer, *temp;  

  cell_producer = (struct cell *) calloc(1, sizeof(struct cell));
  cell_producer->lines = (char **) calloc(NUMLINES, sizeof(char *));

  for (i = 0; i < NUMLINES; ++i) {
  	cell_producer->lines[i] = (char *) calloc(MAXCHAR, sizeof(char));
  }


  // Creem els fils secundaris
  ...
  //PRODUCTOR, aquí comença el fil principal amb la funció de fer de productor
  ...
  while (true) {
  	...
  	cell_producer->nelems = linesTotal;

  	pthread_mutex_lock(&mutexP);
  	

    // En cas de que no n'hi hagin bloc per donar-li al consumidor, s'adormeix
  	while (comptador == BLOCKS) {
  		pthread_cond_wait(&condP, &mutexP);
  	}


    // Fem el canvi de punters per a que sigui el més eficient possible   
  	temp = buffer[w];
  	buffer[w] = cell_producer;
  	cell_producer = temp;
  ...
  //Quan el fil principal espera a que els secundaris hagin processat tots els blocs

  for (i = 0; i < MAXFILS; i++) {
    err = pthread_join(ntid[i], NULL);

    if (err != 0) {
        printf("No puc juntar el fil.\n");
        exit(1);
     }
  }


  //Per últim alliberem tota la memoria utilitzada
